...
async def test_create_list():
    ...
    await c.cmd("RPUSH", "rp_list_key", "foo")
...
async def test_append_element():
    ...
    await c.cmd("RPUSH", "rp_append_key", "a")
    await c.expect(integer(1))
    await c.cmd("RPUSH", "rp_append_key", "b")
    await c.expect(integer(2))
    await c.cmd("RPUSH", "rp_append_key", "c")
...
async def test_append_multiple():
    ...
    await c.cmd("RPUSH", "rp_multi_key", "a", "b", "c")
    await c.expect(integer(3))
    await c.cmd("RPUSH", "rp_multi_key", "d", "e")
    await c.expect(integer(5))

    # a single element through the variadic path still works
    await c.cmd("RPUSH", "rp_multi_key", "f")
...
async def test_wrongtype():
    ...
    await c.cmd("SET", "rp_stringy", "value")
    await c.expect(OK)
    await c.cmd("RPUSH", "rp_stringy", "x")
    await c.expect_error()

    # and the reverse: GET on a list key
    await c.cmd("RPUSH", "rp_listy", "x")
    await c.expect(integer(1))

    # connection still usable
    await c.cmd("RPUSH", "rp_listy", "y")
    await c.expect(integer(2))
    await c.close()
...
async def test_arity():
    ...
    await c.cmd("RPUSH", "rp_after", "errors")
    await c.expect(integer(1))
    await c.close()
...
async def test_awkward_elements():
    ...
    await c.cmd("RPUSH", "rp_awkward_key", "", '"quoted"', "$5", "*2", "a\r\nb", "x" * 500)
    await c.expect(integer(6))
    await c.close()
...
async def test_duplicates():
    ...
    await c.cmd("RPUSH", "rp_dupe_key", "same", "same", "same")
    await c.expect(integer(3))
    await c.cmd("RPUSH", "rp_dupe_key", "same")
...
async def test_expired_key_becomes_fresh_list():
    ...
    await c.cmd("SET", "rp_ttl_key", "v", "PX", "100")
    await c.expect(OK)
    await asyncio.sleep(0.2)

    await c.cmd("RPUSH", "rp_ttl_key", "fresh")
    await c.expect(integer(1))
    await c.close()
...
async def test_rpush_keeps_ttl():
    ...
    await c.cmd("RPUSH", "rp_keep_key", "a")
    await c.expect(integer(1))
    ...
    await c.cmd("RPUSH", "rp_keep_key", "b")
    await c.expect(integer(2))
    await c.close()
...
async def test_shared_across_connections():
    ...
    await a.cmd("RPUSH", "rp_shared_list", "a", "b")
    await a.expect(integer(2))
    await a.close()

    b = await Client("reader").connect()
    await b.cmd("RPUSH", "rp_shared_list", "c")
...
async def test_pipelined():
    ...
    await c.send(array("RPUSH", "rp_pipe_key", "a") + array("RPUSH", "rp_pipe_key", "b"))
    await c.expect(integer(1))
    await c.expect(integer(2))
    await c.close()
...
async def test_concurrent():
    ...
    async def one(n: int):
        ...
        await c.cmd("RPUSH", f"rp_burst_key_{n}", "x", "y")
        await c.expect(integer(2))
        await c.cmd("RPUSH", f"rp_burst_key_{n}", "z")
        await c.expect(integer(3))
        await c.close()
...
async def test_string_still_readable():
    ...
    await c.cmd("SET", "rp_intact_key", "original")
    await c.expect(OK)
    await c.cmd("RPUSH", "rp_intact_key", "x")
    await c.expect_error()
    await c.cmd("GET", "rp_intact_key")
